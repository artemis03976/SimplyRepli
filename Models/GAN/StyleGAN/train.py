...
def train(config, model, train_loader):
    ...
    criterion_generator = GeneratorLoss(config)
    criterion_discriminator = DiscriminatorLoss(config)
...
def train_step(model, config, train_info, criterion, optimizer):
    ...
    for batch_idx, (image, _) in enumerate(train_info):
        ...
        for _ in range(config.d_step):
            ...
            loss_discriminator = criterion_discriminator(discriminator, output_real, output_fake, real_data, fake_data)
        ...
        for _ in range(config.g_step):
            ...
            loss_generator = criterion_generator(output_fake)
